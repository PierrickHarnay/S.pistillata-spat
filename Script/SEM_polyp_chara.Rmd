...
#remove column 2,3,4 in the dataset letters_corallite 
...
summary_primary <- summary_primary[, -c(4,5)]
...
# Merge both
Corallite_Columella_point_plot <- merge(Corallite_Columella_point_select, letters_corallite, by = "Polyp_number", all.x = TRUE)
print(Corallite_Columella_point_plot)

# Ensure columns have the correct types
# X-axis as factor
Corallite_Columella_point_plot$Polyp_number <- factor(Corallite_Columella_point_plot$Polyp_number)          
# Y-axis numeric
Corallite_Columella_point_plot$Corallite_Diameter <- as.numeric(Corallite_Columella_point_plot$Corallite_Diameter)  
# Label as character
Corallite_Columella_point_plot$Letters <- as.character(Corallite_Columella_point_plot$Letters)  

# Compute max value per Polyp_Number to position letters above the boxes
label_positions <- Corallite_Columella_point_plot %>%
  group_by(Polyp_number) %>%
  summarise(y_pos = max(Corallite_Diameter, na.rm = TRUE))

# Merge the letters (if not already)
label_positions <- merge(label_positions, Corallite_Columella_point_plot[, c("Polyp_number", "Letters")], by = "Polyp_number", all.x = TRUE)

#corallite
Corallite_plot <- ggplot(Corallite_Columella_point_select, aes(x = factor(Polyp_number), y = Corallite_Diameter))+
  geom_boxplot(outlier.shape = NA, fill = "darkslategray3", width = 0.5, alpha = 0.5) +  
  geom_jitter(width = 0.5, color = "black", size = 1, alpha = 0.5) + 
  geom_text(data = label_positions,
            aes(x = Polyp_number, y = y_pos + 2, label = Letters), 
...
#ploting Corallite diameter with the data point and not the mean of it 
# Merge both
Corallite_Columella_point_plot <- merge(Corallite_Columella_point_select, letters_columella, by = "Polyp_number", all.x = TRUE)
print(Corallite_Columella_point_plot)
...
Corallite_Columella_point_plot$Polyp_number <- factor(Corallite_Columella_point_plot$Polyp_number)          
# Y-axis numeric
Corallite_Columella_point_plot$Columella <- as.numeric(Corallite_Columella_point_plot$Columella)  
# Label as character
Corallite_Columella_point_plot$Letters <- as.character(Corallite_Columella_point_plot$Letters)  
...
Columella_plot <- ggplot(Corallite_Columella_point_select, aes(x = factor(Polyp_number), y = Columella))+
  geom_boxplot(outlier.shape = NA, fill = "darkslategray3", width = 0.5, alpha = 0.5) +  
  geom_jitter(width = 0.5, color = "black", size = 1, alpha = 0.5) + 
  labs(x = "Polyp number", y = "Columella diameter (µm)") +
  geom_text(data = label_positions,
