...
CAN START DIRETCLY AT LINE 133!!!
ALL THE STEP BETWEEN NOW AND 133 ARE FOR GENERATE THE DATA FILE BY CREATING LOOP AND EXTRACTING THE DATA TO START THE ANALYSIS
...
```{r, warning=FALSE, message=FALSE}
path.p<-"Data/Respirometry/Plates" #location of files
```
...
```{r, warning=FALSE, message=FALSE}
Sample.Info<-read.csv("Data/Spat_Info.csv") #information on life stage, number of individuals, runs, etc.
...
starttimes<-read.csv("Data/Respirometry/starttimes.csv") #read in file of starting times of the oxygen measurements
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("Data/Respirometry/RespirationPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
write.csv(Resp.R, paste0("Data/Respirometry/Respiration_rates_Spis_May2023.csv", sep="")) #save respiration rate data
```
...
```{r, warning=FALSE, message=FALSE}
Resp.Rates <- read.csv(file = "Data/Respirometry/Respiration_rates_Spis_May2023.csv") #read file back in so slopes don't have to be generated every time
...
```{r, warning=FALSE, message=FALSE}
Sample.Info$Sample.ID <- as.character(Sample.Info$Sample.ID) #make sample id into character
Resp.Rates$Sample.ID  <- as.character(Resp.Rates$Sample.ID) #make sample id into character
...
Vizualisation of the data per plates 
```{r}
...
```
Need to remove the outlier at the low end and also keep only negative values because in the dark positive values (photosyntheis) of respiration rate (draw down of Oxygen) should not be possible.

Normalize to biologically relevant measure. Here, normalize to size of spat, or larval size or volume as well.  
```{r, warning=FALSE, message=FALSE}
...
```
Plotting the data as a regression of respiration as a function of Polyp number
```{r}
ggplot(oxygen.bio, aes(R.nmol.mm2.min, Plate.ID,)) + 
  geom_point()
...
p1 <- ggplot(data = oxygen.bio, aes(x = as.numeric(Polyp.Number), y = R.nmol.mm2.min)) +
  stat_poly_line(color = "cyan2", linewidth = 1, se = TRUE, 
                 fill = "darkslategray3", alpha = 0.2) +
  stat_poly_eq(use_label(c("eq", "R2","P")), size = 4) +
  geom_point(color = "darkgray", size = 2, stroke = 0.7, shape = 21, fill = "darkgray") +
  theme_classic(base_size = 14)

ggsave("Output/respiration_polyp_number.png", plot = p1, width = 8, height = 6, dpi = 300)


resp.polyp.lm <- lm(R.nmol.mm2.min~Polyp.Number, data = oxygen.bio)
...
p1 <- ggplot(data = oxygen.bio, aes(x = Size.mm2, y = R.nmol.mm2.min)) +
  stat_poly_line(color = "cyan2", linewidth = 1, se = TRUE, 
                 fill = "darkslategray3", alpha = 0.2) +
  stat_poly_eq(use_label(c("eq", "R2","P")), size = 4) +
  geom_point(color = "darkgray", size = 2, stroke = 0.7, shape = 21, fill = "darkgray") +
...
ggsave("Output/size_mm2.png", plot = p1, width = 8, height = 6, dpi = 300)
...
p2 <- ggplot(oxygen.bio, aes(x = log10(Size.mm2 + 1), y = log10(R.nmol.mm2.min + 1))) +
  stat_poly_line(color = "cyan2", linewidth = 1, se = TRUE, 
                 fill = "darkslategray3", alpha = 0.2) +   # blue line + transparent (fill) SE ribbon
  stat_poly_eq(use_label(c("eq", "R2", "P")), size = 4) +
  geom_point(color = "darkgray", size = 2, stroke = 0.7, shape = 21, fill = "darkgray") + # clear points
  theme_classic(base_size = 14)

ggsave("Output/size_mm2_+1.png", plot = p2, width = 8, height = 6, dpi = 300)
```
Comparing the 3 model and using the R2 we can notice that the model of number of polyp is the a great model because R2=<0.01 so it's mean we force the the model and the mean can be enough 
The R2=0.06 explain only 6% of the variation of our oxygen data and it can be because of the noice too prononce in the data or just not a clear linear trend. This is the R2 for sizes 

Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("Output/Spis_spat_RespRates.csv", sep="")) #save final file
```

Statistic of the linear model 

testing the assumptions of linear regression Distribution of the data
```{r, warning=FALSE, message=FALSE}

#model 
model <- lm((R.nmol.mm2.min+1)^0.333 ~ oxygen.bio$Size.mm2, data = oxygen.bio)

model$residuals

#Histogram vizualisation for normality of data
hist(model$residuals, main = "Histogram of Respiration Rates",
...
# Fit Theil-Sen regression (median-based linear model)
modelTS <- mblm(R.nmol.mm2.min ~ Size.mm2, repeated = FALSE, data = oxygen.bio)  # Use repeated = TRUE for Siegel’s method
...
# Plot the regression
plot(oxygen.bio$Size.mm2, oxygen.bio$R.nmol.mm2.min)
...
```
#the shapiro test confirm to us that the data is not normal (p-value= 6.293e-06, reject H0) and we will need to do an transformation for the next step
```{r, warning=FALSE, message=FALSE}
#log10 transformation 
oxygen.bio$R_log10 <- log10(oxygen.bio$R.nmol.mm2.min + 1)
oxygen.bio$Size_log10 <- log10(oxygen.bio$Size.mm2 + 1)

```

#check of the normality with a shapiro test 
```{r, warning=FALSE, message=FALSE}
# Check normality after log10 transformation
shapiro.test(log10(oxygen.bio$R.nmol.mm2.min + 1))
shapiro.test(log10(oxygen.bio$Size.mm2 + 1))

```

#homogeneity of variance  with heteroskedasticity 
```{r, warning=FALSE, message=FALSE}
model <- lm(R_log10 ~ Size_log10, data = oxygen.bio)
plot(model, which = 1)  # Residuals vs Fitted

```
#because my data looks heteroskedasticity = spread increases or decreases we will do a Breusch-Pagan Test
```{r, warning=FALSE, message=FALSE}
library(lmtest)
bptest(model)
```
because the p-value is significant, we will use a robust standard errors because heteroskedasticity 
```{r, warning=FALSE, message=FALSE}
library(sandwich)
