...
```{r, warning=FALSE, message=FALSE}
###### BECAUSE SAMPLE.INFO IS AN CHARACTER AND NOT RESP.RATES SOME CHANGE NEED TO APPEAR #########

Sample.Info$Sample.ID <- as.character(Sample.Info$Sample.ID)
Resp.Rates$Sample.ID  <- as.character(Resp.Rates$Sample.ID)
...
```
Need to remive the outlier who shift the trend and also remve the negative value because erelevant on what we want to do
...
```
Ploting the data with the different function with a regression as Polyp number/ size 
```{r}
ggplot(oxygen.bio, aes(R.nmol.mm2.min, Plate.ID,)) + 
  geom_point()
...
hist(oxygen.bio$R.nmol.mm2.min, main = "Histogram of Respiration Rates",
     xlab = "R.nmol.mm2.min", col = "lightblue", breaks = 15)

#QQ-plot
qqnorm(oxygen.bio$R.nmol.mm2.min)
qqline(oxygen.bio$R.nmol.mm2.min, col = "red", lwd = 2)
```
...
```{r, warning=FALSE, message=FALSE}
shapiro.test(oxygen.bio$R.nmol.mm2.min)
